...
[w-gore, s-dolje, d-desno, a-lijevo]''' )
    ...
    def postavljanje_srednjeg_broda_racunalo(self):
        ...
        while(ponovi):
            ...
            if (smjer==1):                  
                ...
                d=p
                t=p
            self.racunalo.zauzeta_polja+=(str(p)+str(pp))+(str(d)+str(dp))+(str(t)+str(tp))
            self.racunalo.zauzeta_polja[2:4]=[''.join(self.racunalo.zauzeta_polja[2:4])]
    ...
    def postavljanje_velikog_broda_racunalo(self):
        ...
        while(ponovi):
            ...
            if (neispravan_unos==False):
                self.racunalo.zauzeta_polja+=(str(p)+str(pp))+(str(d)+str(dp))+(str(t)+str(tp)+(str(c)+str(cp)))
                self.racunalo.zauzeta_polja[5:7]=[''.join(self.racunalo.zauzeta_polja[5:7])]
                self.racunalo.zauzeta_polja[6:8]=[''.join(self.racunalo.zauzeta_polja[6:8])]
            ...
            for i in self.racunalo.zauzeta_polja[0:5]:
                for j in self.racunalo.zauzeta_polja[5:]:
                    #print(i+j)
                    if (i==j):
                        del self.racunalo.zauzeta_polja[5:]
                        ponovi=True
                        break
    ...
    def igranje(self):
        ...
        self.unosIgraca()
        while True:
            if self.covjek.brodoviSuPotopljeni()  == 3 or self.racunalo.brodoviSuPotopljeni()  == 3:
                break
